...
#### 2.2.1 안내 영역
- **공지사항 / 업데이트 / 고객센터 / +Plus Membership / 이벤트**를 포함
- 모든 서비스 기능의 운영 메시지를 전달하는 영역  
- 관리자 기능과 사용자 피드백 기능이 명확히 구분됨

#### 2.2.2 커뮤니티 시스템
- **명예의전당 / 출석체크 / 자유게시판 / 유머재미 / 질문답변**
- 활동 기반 포인트 획득  
...
#### 2.2.4 분양자료 시스템
- **Data Center / 시장조사서 / 제안서 / 교육자료 / 정책자료 / 기타자료 / Ai보고서**
- Data Center는 각종 통계와 리서치, 크롤링을 통한 순수 data 구성 및 청약홈 분양공고 현장과 연동 된 전체 현장별 DATA 테이블 구성
- 시장조사서, 제안서, 교육자료, 정책자료, 기타자료는 페이지별로 권한이 존재하며 업/다운로드 하는 구성
- Ai 보고서는 Ai 기반 자동 리포트 생성 (PDF 출력 가능)  
- (공통) 자료 업로드 → 포인트 지급 / 다운로드 → 포인트 차감  
- 시장조사서, 제안서는 관리자가 실무자 승인한 계정만 다운로드 이용 가능 방식(업로드는 허용 된 회원 유형만 가능) + 자동 필터 + 중복 방지 로직 포함
- 시장조사서, 제안서 페이지는 특수한 페이지야. 페이지를 보는거는 모두가 가능하지만, 업로드는 회원유형이 분양기획+관계사일 때만 허용, 게시물 모달 열기, 장바구니 담기, 다운로드는  분양기획+관계사 중 실무자승인 된 회원만 허용.
...
5. **구인/구직 연결**
   - 데이터센터에 있는 모집공고 리스트 탐색 / 조건 필터 / 문서 다운로드  
...
- **+premium Membership**  
  → plus 혜택 모두 포함 + Ai 보고서 토큰 월 N개 지급  
  → 구인글 상단노출권 제공
  → 광고 무시권 제공  
  → 포인트 충전 N% 제공
  → 상점 할인 N%
...
- 관리자 대시보드에서 다음 항목 추적 가능:
  - 기본적인 유저 정보 / 유저수 / 신규 가입 추이 / 재방문 추이 / 평균 접속 유지 시간
  - 유료플랜 가입 추이 / 포인트 충전 판매량 / 광고 시청률  
...
> 드롭다운 형식으로 상세 정보는 클릭하여 확인할 수 있도록 화면 설계
...
- **카피라이트 문구**:  
  > `© 마케팅인사이트(WAVE space) All rights reserved.`

- **푸터 배경**: 연회색 (#F9FAFB) 또는 화이트  
- **폰트 크기**: 13px / 줄간격 1.4em  
- **정렬 방식**: 카피라이트 문구 드롭다운 시 좌측 회사 정보 / 우측 링크 그룹 
...
- **UX 흐름**:
  ...
  - CTA 버튼: '지원하기' (버튼 입력 시 구인공고 게시자에게 알림)
  - 포인트 기반 상단 노출 기능 포함
- **관리자 기능**:
  - 비공개 전환 / 모집상태 변경 / 마감 자동처리
  - 공고별 열람통계 / 지원자 리스트 출력
...
- 스티커 기능: '급구', '첫투입', '수수료인상', '조건변경' 등 5~8종의 비주얼 라벨
- 스티커는 기본 무료 + 유료 구분 / 정렬 우선순위에도 영향
...
- CTA 버튼:  `상세보기`, `공고 찜`, `문의하기`
- 홈화면 인기 공고 요약 섹션과 연동 → 일정 조건 충족 시 자동 추천 노출

##### 연동 구조

- Data Center의 단지 정보와 공고 단지명이 반드시 일치해야 등록 가능 (검색창에 단지명 조회)
...
- **기능 설명**:
  - 개인은 ai 매칭 서비스 등록
  - 기업은 ai 매칭 서비스 이용
  - 기업이 원하는 조건을 선택하면 최적의 2명 인력 추천
  - 기업에게 ai 매칭 되는 인력의 프로필은 관리자가 허용한 부분만 공개(일부 비공개 처리 관리)
  - Ai 매칭 서비스는 포인트 소진 (BETA 기간에는 포인트 미소진, 1주 1회 횟수 제한으로 운영)
  - Ai 추천 2명에게 기업은 입사지원요청 알림을 보낼수 있음
  - 관리자 패널에서 직접 매칭하거나 '추천 등록' 기능 사용
  - AI매칭 등록 후 공개/비공개 설정 가능
- **UX 흐름**:
  - 개인 입장에서는 '프로필 등록' 기능으로 진입
  - 회사 채용자 입장에서는 'ai 매칭 서비스' 기능으로 진입
- **관리자 기능**:
  - 인재풀 관리 / 키워드 기반 필터링 / 채용 히스토리 저장
  - 추천 성공시 개인에게는 보상 지급 설정 가능 (예: 쿠폰 / 포인트 / 자유이용권)
...
- **포함 항목**:
  - 탭1 - 전국 시도별 주소 간편 조회 / 아파트 공급 현황 / 미분양 현황, 거래량, 입주물량, 가격동향, 인구동향 / 추이 데이터 테이블 및 그래프  
  - 탭2 - 네이버 부동산 뉴스 (헤드라인, 일부 내용 제공, 클릭 시 네이버 페이지 브라우저 새로 열림)
  - 탭3 - 현장별 공급개요 / 분양률 / 대행사명 / 계약조건 / 영업형태 / 영업수수료  
  - 영업수수료는 분양영업 공고와 자동 연동됨
  - 탭4 - 청약홈 연동 데이터 (신규 단지, 모집공고 요약, 경쟁률 등) / 특정 표형식의 정리된 포맷 제공
...
- 사용자 업/다운로드 가능  
- **조회 기능**:
  - 지역 상세조회: 광역시도, 시/군/구 선택
  - 상품유형별 조회: 태그형 필터(아파트, 주거형OT, 수익형OT, 도생, 상가, 생활형숙박시설, 지식산업센터, 오피스, 기타) + 아이콘  
  - 키워드 조회: 자동완성 + 추천 키워드 리스트  
  - 최근 검색 저장 기능 포함  
  - 게시물 제목 클릭 시 모달로 썸네일과 다운로드 파일 제공
  - 시장조사서, 제안서 페이지는 특수한 페이지야. 페이지를 보는거는 모두가 가능하지만, 업로드는 회원유형이 분양기획+관계사일 때만 허용, 게시물 모달 열기, 장바구니 담기, 다운로드는  분양기획+관계사 중 실무자승인 된 회원만 허용.
  - 게시물 신고기능 포함

- **자료 등록 기준**  
  - 2년(24개월) 이내 문서만 등록 가능  
  - 지역 상세입력: 시/군/구 체크박스 (대분류에 따라서 중분류가 변경되도록 로직 구성)
  - 상품유형별 선택: 체크박스(아파트, 주거형OT, 수익형OT, 도생, 상가, 생활형숙박시설, 지식산업센터, 오피스, 기타)
  - 분양유형별 선택: 민간분양, 공공분양, 민간임대, 공공임대, 기타
  - 자료 생성일 선택: 체크박스로 연도,월,일 선택 가능하도록 구성 (기본 세팅은 2023년부터 2030년까지) / 자료중복 시 중복자료 내역 알림 팝업 / 중복 업로드 불가
  - 자료 미리보기 자동 등록: 5번째 페이지만 보여지도록 표시
  - 키워드 최소 10개이상 자동 등록 / 키워드 예시 학세권, 역세권 등 (키워드로 검색창 조회)
  - 파일크기 자동 입력
  - 문서 자동 중복 검수 (지역1,2(광역시도+시군구), 파일크기, 페이지수, 파일생성일, 상품유형, 분양유형 이렇게 같을 시 중복자료로 업로드 불가)
    - 업로드 불가시에는 기존에 먼저 업로드 된 동일 자료에 대해서 내역을 알림 팝업.
  - 시장조사서 최신성 + 페이지수 기준 포인트 보상 차등 지급 (업로드 당일 기준으로)
    - 다운로드 또한 아래 기준에 동일하게 적용
    - 6개월 이내: 120%  
    - 1년 이내: 100%  
    - 2년 이내: 70%
    - 2년 경과: 업로드불가 
  - 페이지 기준 포인트 보상 차등 지급
    ...
    - 10~19페이지: 90%
    - 10페이지 미만: 60%
  - 제안서는 최신성 + 페이지수 기준 포인트 보상 차등 지급 (업로드 당일 기준으로)
    - 6개월 이내: 120%  
    - 1년 이내: 100%  
    - 2년 이내: 70%
    - 2년 경과: 업로드불가 
  ...
  - 교육자료는 최신성 기준으로만 포인트 보상 차등 지급 (업로드 당일 기준으로)

- **폴더 접근 권한 관리**  
  - 관리자 페이지에서 등록, 다운로드, 읽기, 댓글, 삭제 권한 부여 관리
  - 열람 권한: 전체공개 / 회원유형별 다운로드 제한  
...
- 태그 기반 정렬 / 검색 기능은 시장조사서와 동일  
- 자유형 문서에 자동 키워드 추천 시스템 연동

#### 6.5.4 AI 보고서

- 주소 또는 단지명 입력 → 하위에 자동 리포트 생성  
...
뱃지는 닉네임 오른쪽에 위치시키도록 한다.

### 8.2 분류 및 기능 목적

#### 8.2.1 활동 기반 뱃지
- 출첵리더, 소통리더, 답변리더, 정보리더, 추천리더
- 월간 누적 활동을 기준으로 1위에게 자동 발급
...
- 뱃지는 목록을 30개 만들어 놓고 획득시 채워지게 표현, 미획득은 흐리게 표시
- 명예의 전당은 월계관 + 뱃지 그룹 구성
- 뱃지 디자인은 텍스트로만 하되, 텍스트의 형식을 일반, 희귀귀, 프리미엄으로 등급을 나누어 글씨색을 다르게 표시. 회색,하늘색,컬러그라데이션색
- 스타일 종류:
  - 일반: 회색 텍스트
  - 희귀: 짙은 회색 텍스트
  - 프리미엄: 짙은 회색 텍스트 / 깜박이기 효과
  - 스페셜: 연한 컬러 텍스트 / 타이핑효과
- 표시방식 "ㅣ유머로다스린다ㅣ" 이런 형식으로 "ㅣ내용ㅣ" 하고, 글자색은 기본 짙은 회색
...
- 미니게임 광고무시권 / 미니게임 추가참여 / 전용 아이템 잠금해제
...
- 커뮤니티 활동: 글쓰기, 출석, 댓글, 추천, 질문답변 채택
- 데이터센터 활동: 분양단지 분양률 및 히스토리 등 정보 입력 시
- 뱃지 획득 / 포인트 랭킹 진입 / 명예의 전당 등재
- 시즌 이벤트 또는 챌린지 참여 완료

#### 9.2.2 리워드 보너스
- 일일 미션 성공 시 랜덤 보상 지급 (예: 댓글 3개 → +30P)
- 요일별 지급 정책 (금요일 업로드 보상 1.1배 등)
- 시간대별 지급 정책 (오전 9시~12시 업로드 보상 1.1배)
...
- 1천천Token = 1,000P로 구매 가능 / 이벤트 보상으로 획득 가능
...
- 아이템 카테고리: 미니게임 자유이용권, 네임카드 꾸미기, Token, 뱃지, 시즌한정
- 일부 아이템은 뱃지 또는 VIP 조건 충족 후 구매 가능
- 작은 카드형형 UI / 월별 구매 제한 / 잔여 수량 / 할인율 표시 / 즉시 사용 or 보관함 선택
- 할인율은 특정 뱃지 보유 시 추가 할인이 적용되는 방식으로 자동 조정됨
...
##### 9.8.2.5 쿠폰 아이템
- 기능: 포인트 구매 할인 / 상점 아이템 할인 / 리포트 자동 생성권 / 제휴 서비스 할인 / 파일다운로드 할인 / 파일업로드 획득 시 추가보상(1%,3%,5%,10%)
- 획득 방식: 이벤트 보상 or 마켓 제휴 코드 입력
- 사용 시점: 즉시 적용 or 보관함 지정

##### 9.8.2.6 프라이빗 아이템
- 닉네임 비공개: 사용시 닉네임 비공개 선택 (출석체크, 자유게시판, 유머재미, 구인구직, 분양자료)
- 닉네임은 비공개 되어도 뱃지는 노출되는 것이 원칙
...
- 홈페이지 휴먼계정 처리 시 포인트 일괄 소멸(10만 포인트 이하 시)
- 포인트 기본 유효기간: 365일
...
- 회원 관리
- 분양기획 2단계 관리 (분양기획파트 1단계 일반: 분양기획, 2단계: 기획실무자)
- 콘텐츠 승인/검수
- 포인트 정책 관리
- 뱃지 시스템 운영
- 채용 공고 승인 및 상태 제어
...
#### 10.3.1 기본 항목
- 회원 검색: 닉네임 / 이메일 / 직군 / 가입일 / 활동 등급 / 뱃지 보유 여부 / 전화번호 / 수동등록(회사명, 메모모)
- 상태 보기: 활동중 / 경고 / 정지 / 블라인드 / 탈퇴
- 보유 포인트 / 쿠폰 / Token / 아이템 확인
- 활동 내역: 문서 다운로드, 포인트 사용내역, 게시글 내역
...
##### 주요 알림 뷰
- 오늘 발생 이벤트: 회원수, 접속률, 자료 등록, 구직자 지원, 채용 마감, 신고 접수, 상점판매량, 광고시청량, 전체 포인트 변동량
...
### 11.2 주요 섹션
- 내 프로필: 닉네임, 뱃지(대표 1개 설정), 등급, 직군, 소속, 가입일
- 포인트/토큰 보유 현황
...
### 13.1 목적
- 여러 자료/아이템을 일괄 포인트로 결제하고 관리할 수 있는 공간
...
### 14.3 UI 흐름
- 작은 카드형 보기 / 분류필터 / 최근 사용순 / 만료 아이템 강조
...
- **추가 입력 활성화**:
  - 직급 (체크박스): 주임 / 대리 / 과장 / 차장 / 팀장 / 실장 / 본부장 / 기타  
  - 경력 기간 (숫자 입력)  
...
- **기능 추가 제공**:
  - 실무자 승인 시 시장조사서, 제안서 다운로드 활성화 
  - 실무자 승인 시 닉네임카드 회원유형 분양기획에서 기획실무자로 변경
...
- **기능 제공**:
  - 구인공고 열람 및 지원 / 영업수수료 비교 / 닉네임 카드 분양영업 표시
...
- **기능 제공**:
  - 콘텐츠 열람 / 출석체크 / 커뮤니티 글 작성 / 미니게임 / 포인트 상점  
...
- 화면 상단에는 **탑바가 존재하지 않으며**, 그 대신 콘텐츠 영역 상단 우측에 기능성 UI가 배치됩니다.
  - 이에는 알림 / 닉네임 / 대표 뱃지 / 마이페이지 / 설정 아이콘이 포함됩니다.

- 콘텐츠는 각 기능별로 `카드형`, `리스트형`, `폼형`으로 나뉘며, 포인트 시스템과 UI 연계됩니다.
- 예를 들어:
  - 자유게시판 → 카드형 게시물 + 태그 정렬 필터
  - 분양자료 → 카드형 게시물 + 포인트 차감 UI + 팝업창으로 썸네일과 첨부자료 다운로드 안내
