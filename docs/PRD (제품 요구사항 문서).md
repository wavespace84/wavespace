...
- 시장조사서, 제안서, 교육자료, 정책자료, 기타자료는 회원들이 자유롭게 문서를 업/다운로드 하는 구성
- Ai 보고서는 Ai 기반 자동 리포트 생성 (PDF 출력 가능)  
- (공통) 자료 업로드 → 포인트 지급 / 다운로드 → 포인트 차감  
- 시장조사서, 제안서는 관리자가 실무자 승인한 계정만 다운로드 이용 가능 방식(업로드는 미승인 허용) + 자동 필터 + 중복 방지 로직 포함
...
- **+Plus Membership**  
  → Pro 혜택 모두 포함 + Ai 보고서 토큰 월 10개 지급  
...
- **포함 항목**:
  ...
  - 전국 시도별 미분양 현황, 거래량, 입주물량, 가격동향, 인구동향 / 추이 데이터 테이블 및 그래프  
  - 현장별 공급개요 / 분양률 / 대행사명 / 계약조건 / 영업형태 / 영업수수료  
...
- 사용자 업/다운로드 가능  
- **조회 기능**:
  - 전국지도 표시, 지도에 필터 조건값에 따라 문서수 명시
  - 지역 상세조회: 광역시도, 시/군/구 선택
  - 상품유형별 조회: 태그형 필터(아파트,오피스텔(주거형,수익형),도시형생활주택,상가,지식산업센터,생활형숙박시설,호텔,기타) + 아이콘  
  - 키워드 조회: 자동완성 + 추천 키워드 리스트  
  - 최근 검색 저장 기능 포함  
  - 게시물 제목 클릭 시 팝업창으로 썸네일과 다운로드 파일 제공
  - 신고기능 포함

- **자료 등록 기준**  
  - 2년 이내 문서만 등록 가능  
  - 지역 상세입력: 시/군/구 체크박스 (대분류에 따라서 중분류가 변경되도록 로직 구성)
  - 상품유형별 선택: 체크박스(아파트,오피스텔(주거형,수익형),도시형생활주택,상가,지식산업센터,생활형숙박시설,호텔,기타)
  - 분양유형별 선택: 공공분양, 민간분양, 임대
  - 자료 생성일 선택: 체크박스로 연도,월,일 선택 가능하도록 구성 (기본 세팅은 2023년부터 2030년까지) / 자료중복 시 중복자료 내역 알림 팝업 / 중복 업로드 불가
  - 썸네일 자동 등록: 2개 페이지 왼쪽 1/2만 보여지도록 (미리보기 방지)
  - 키워드 최소 2개이상 선택 등록 (필수) / 키워드 예시 학세권, 역세권
  - 파일크기 자동 입력
  - 문서 자동 중복 검수 (지역1,2(광역시도+시군구), 파일크기, 파일생성일, 상품유형, 분양유형 이렇게 같을 시 중복자료로 업로드 불가)
    - 업로드 불가시에는 기존에 먼저 업로드 된 동일 자료에 대해서 내역을 알림 팝업.
  - 최신성 기준 포인트 보상 차등 지급 (업로드 당일 기준으로)
  - 다운로드 또한 아래 기준에 동일하게 적용용
    - 6개월 이내: 130%  
    - 1년 이내: 100%  
    - 1년 경과: 70%
  - 파일크기 기준 포인트 보상 차등 지급
    - 5MB 이상: 110%
    - 2~5MB미만: 100%
    - 2MB미만: 70% 
