...
function getBadgeIcon(badgeText) {
    const badgeMap = {
        소통리더: '<i class="fas fa-comments" style="color: #FF6B6B;"></i>',
...
document.addEventListener('DOMContentLoaded', () => {
            ...
            if (!isCheckedIn) {
                try {
                    ...
                    if (window.showErrorMessage) {
                        showErrorMessage('출석 처리 중 오류가 발생했습니다.');
                    } else {
    ...
    const currentMonth = getServerTime(); // 서버 시간 기준
...
function initializeStatistics() {
    ...
    const attendanceRate =
        lastDayOfMonth > 0 ? Math.round((monthlyCount / lastDayOfMonth) * 100) : 0;

    // 통계 카드들을 왼쪽부터 순서대로 애니메이션
    const cards = [
        {
            selector: '.streak-info .streak-card:nth-child(1) .streak-count',
            value: monthlyCount,
            suffix: '일',
            delay: 0,
        },
        {
            selector: '.streak-info .streak-card:nth-child(2) .streak-count',
            value: streakCount,
            suffix: '일',
            delay: 300,
        },
        {
            selector: '.streak-info .streak-card:nth-child(3) .streak-count',
            value: totalPoints,
            suffix: 'P',
            delay: 600,
        },
        {
            selector: '.streak-info .streak-card:nth-child(4) .streak-count',
...
function performCheckIn() {
    ...
    setTimeout(() => {
        ...
        // 출석 보상 카드 업데이트 - 약간의 지연 후 실행
        setTimeout(() => {
            console.log('출석 보상 카드 업데이트 실행 - 순위:', currentUser.rank);
            updateRewardCards();
        }, 100);
    ...
    if (isCheckedIn) {
        ...
        if (checkStatus) {
            checkStatus.innerHTML =
                '<i class="fas fa-check-circle"></i> <span>오늘 출석 완료!</span>';
        ...
        if (checkStatus) {
            checkStatus.innerHTML =
                '<i class="fas fa-clock"></i> <span>아직 출석하지 않았습니다</span>';
...
function updateAllStatisticsAfterCheckIn() {
    ...
    const monthlyElement = document.querySelector(
        '.streak-info .streak-card:nth-child(1) .streak-count'
    );
    if (monthlyElement) {
    ...
    const streakElement = document.querySelector(
        '.streak-info .streak-card:nth-child(2) .streak-count'
    );
    if (streakElement) {
    ...
    const pointsElement = document.querySelector(
        '.streak-info .streak-card:nth-child(3) .streak-count'
    );
    if (pointsElement) {
        const totalPoints = monthlyCount * 500;
    ...
    const rateElement = document.querySelector(
        '.streak-info .streak-card:nth-child(4) .streak-count'
    );
    if (rateElement) {
...
function updateStatistics() {
    // 이번달 출석일수는 updateAllStatisticsAfterCheckIn에서 이미 처리되므로
    // 여기서는 제거하여 중복 카운팅 방지
    ...
    // 출석률은 updateAllStatisticsAfterCheckIn에서 이미 처리되므로
    // 여기서는 제거하여 중복 계산 방지
}
...
function updateCalendar(date, withAnimation = true, direction = 'next') {
    ...
    if (calendarTitle) {
        ...
        if (isCalendarAnimating && withAnimation) {
            // 모든 이전 wrapper 즉시 제거
            const oldWrappers = calendarTitle.querySelectorAll(
                '.calendar-title-wrapper:not(.current)'
            );
...
function updateRewardCards() {
    // 출석했는지 확인 - 서버 시간 사용으로 수정
    const today = getTodayDate(); // 서버 시간 기준
    const lastCheckIn = localStorage.getItem('lastCheckIn');
    const isCheckedInToday = lastCheckIn === today;

    console.log('출석 보상 카드 업데이트 - 오늘 출석:', isCheckedInToday, '현재 순위:', currentUser.rank);
    ...
    rewardCards.forEach((card, index) => {
        ...
        if (isCheckedInToday && currentUser.rank) {
...
function updateAllStatistics(dates) {
    ...
    // 이번달 출석일수 표시
    const monthlyCountElement = document.querySelector(
        '.streak-info .streak-card:nth-child(1) .streak-count'
    );
