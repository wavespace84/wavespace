/**
 * WAVE SPACE - SMS 인증 서비스
 * Firebase를 이용한 SMS 인증 및 reCAPTCHA 관리
 */
...
// 전역 접근 가능하도록 설정
window.SMSAuth = smsAuthService;

// DOM 로드 완료 후 자동으로 reCAPTCHA 설정
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        setTimeout(async () => {
            if (window.Firebase && window.Firebase.initialize) {
                window.Firebase.initialize();
            }
            await smsAuthService.setupRecaptcha();
        }, 100);
    });
} else {
