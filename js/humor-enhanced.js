    ...
    daily: [
        {
            ...
            content:
                "고객님이 '이 집 발코니가 왜 이렇게 좁냐'고 하셔서 확인해보니... 신발장을 발코니로 착각하고 계셨습니다 ㅋㅋㅋ",
            image: null,
        },
        {
        ...
        },
        {
            title: '분양 현장 웃픈 에피소드',
            content:
                "고객님이 '여기 학군이 어떻게 되냐'고 물어보셔서 열심히 설명했더니... '아 저는 자녀가 없어서요' ...그럼 왜 물어보신거죠?",
            image: null,
        },
    ],
    estate: [
    ...
    story: [
        {
            title: '신입 시절 최악의 실수',
            content:
                "첫 계약을 앞두고 너무 긴장한 나머지... 고객님 성함을 계약서에 반대로 적었습니다. '김철수'님을 '수철김'으로... 다시는 안 그럴게요 ㅠㅠ",
            image: null,
        },
        {
            title: '감동적인 고객 이야기',
            content:
                "10년 동안 청약 떨어지신 고객님이 드디어 당첨되셨을 때, 저한테 전화하셔서 우시더라고요. '덕분입니다'라고... 저도 같이 울었습니다.",
            image: '😭',
        },
        {
            title: '역대급 진상 고객',
            content:
                "모델하우스 관람 3시간, 질문 100개, 자료 요청 50개... 그리고 마지막 한마디 '그냥 구경 왔어요' ... 미소는 유지했지만 속은 무너졌습니다.",
            image: null,
        },
        {
            title: '첫 계약 성공 스토리',
            content:
                "6개월간 한 건도 못하고 있다가... 포기하려던 날 걸려온 전화 한 통. '그때 상담받은 사람인데 계약하고 싶어요' 그날 치킨 10마리 쏘았습니다!",
            image: '🍗',
        },
        {
            title: '가장 기억에 남는 실수',
            content:
                "프레젠테이션 중에 '이 지역은 앞으로 발전 가능성이 없습니다'라고 말했어야 하는데 '많습니다'를 빼먹었어요... 다행히 고객님들이 웃어주셨지만...",
            image: null,
        },
    ],
    tip: [
...
document.addEventListener('DOMContentLoaded', () => {
        ...
        for (const file of files) {
            ...
            if (file.size > 10 * 1024 * 1024) {
                if (window.showWarningMessage) {
                    showWarningMessage('파일 크기는 10MB를 초과할 수 없습니다.');
                } else {
            ...
            if (!file.type.startsWith('image/') && !file.type.startsWith('video/')) {
                if (window.showWarningMessage) {
                    showWarningMessage('이미지 또는 동영상 파일만 업로드 가능합니다.');
                } else {
    ...
    function addPreviewItem(file) {
        ...
        if (file.type.startsWith('image/')) {
            ...
            thumbnail.innerHTML =
                '<i class="fas fa-video" style="font-size: 24px; color: var(--gray-500);"></i>';
    ...
    if (uploadForm) {
        uploadForm.addEventListener('submit', (e) => {
            ...
            if (window.showSuccessMessage) {
                showSuccessMessage('유머 콘텐츠가 성공적으로 업로드되었습니다!');
            } else {
    ...
    function addNewPostToDOM(post) {
        const postsGrid =
            document.querySelector('.posts-grid') || document.querySelector('.humor-grid');
        if (!postsGrid) return;
    ...
    function openPostDetail(index) {
        ...
        document.getElementById('postDetailComments').textContent = post.comments.replace(
            /[^\d]/g,
            ''
        );
        ...
        if (post.image && !post.image.includes('placeholder') && post.image !== '') {
            ...
            if (!placeholderIcon) {
                ...
                placeholderIcon.innerHTML =
                    '<i class="fas fa-image" style="font-size: 72px; color: var(--gray-400); opacity: 0.5;"></i>';
    ...
    function loadComments() {
        ...
        commentsList.innerHTML = sampleComments
            .map(
                (comment) => `
            <div class="comment-item">
    ...
    document.addEventListener('click', (e) => {
        ...
        if (
            postItem &&
            !e.target.closest('.floating-upload-btn') &&
    ...
    if (likeBtn) {
        likeBtn.addEventListener('click', function () {
            ...
            likesSpan.textContent = this.classList.contains('active')
                ? currentLikes + 1
                : currentLikes - 1;
        });
    }
