// 분양기획 구인구직 페이지 JavaScript

// 네비게이션 관련 변수
let currentJobIndex = -1;
let currentFilteredJobs = [];

// 🔗 중앙화된 데이터 사용 (common-data.js)
// regionData는 window.WaveSpaceData.regionData에서 가져옴
...
const jobListings = [
    ...
    },
    {
        ...
        employment: '프리랜서',
        description:
            '아이파크 의정부 프로젝트 사이드 업무를 담당할 분을 모집합니다.\n\n[업무내용]\n- 견본주택 고객 응대\n- 분양상담 보조\n- 고객 안내 및 홍보\n- 이벤트 진행 보조\n\n[근무조건]\n- 프리랜서 계약\n- 시간당 15,000원\n- 성과급 별도 (건별 50만원)\n- 주말 근무 필수\n\n[자격요건]\n- 경력무관 (신입 가능)\n- 고객 응대 서비스 마인드\n- 의정부/양주 거주자 우대\n\n[지원방법]\n- 전화 문의 후 즉시 면접\n- 간단한 이력서 지참',
        tags: ['사이드', '경력무관', '즉시근무'],
...
function initializeFilters() {
    ...
    // 분양유형 선택 checkbox-tab 이벤트 리스너 (라디오 버튼)
            ...
            btn.addEventListener('click', (e) => {
                ...
                if (parts.length === 2) {
                    const checkbox = document.querySelector(
                        `input[name="subregion"][value="${parts[1]}"]`
                    );
                    if (checkbox) {
...
function handleJobListClick(e) {
    ...
    if (target.closest('.btn-bookmark') || target.closest('.btn-bookmark-small')) {
        ...
        const bookmarkBtn =
            target.closest('.btn-bookmark') || target.closest('.btn-bookmark-small');
...
function showJobDetail(jobId) {
    ...
    // 현재 필터된 구인공고 목록 가져오기
    currentFilteredJobs = getCurrentFilteredJobs();
    
    // 현재 구인공고의 인덱스 찾기
    currentJobIndex = currentFilteredJobs.findIndex(j => j.id == jobId);
    ...
    const popupHTML = `
        <div class="job-detail-overlay" onclick="closeJobDetail()">
            <div class="job-detail-popup" onclick="event.stopPropagation()">
                <div class="modal-nav-buttons" style="position: absolute; top: 20px; right: 20px; z-index: 10;">
                    <button class="modal-nav-btn" id="prevJobBtn" onclick="navigateJob('prev')" title="이전 구인공고" disabled>
                        <i class="fas fa-chevron-left"></i>
                    </button>
                    <button class="modal-nav-btn" id="nextJobBtn" onclick="navigateJob('next')" title="다음 구인공고">
                        <i class="fas fa-chevron-right"></i>
                    </button>
                    <button class="modal-nav-btn modal-close" onclick="closeJobDetail()" title="닫기">
...
function navigateJob(direction) {
    if (!currentFilteredJobs || currentFilteredJobs.length === 0) return;
    
    if (direction === 'prev' && currentJobIndex > 0) {
        currentJobIndex--;
    } else if (direction === 'next' && currentJobIndex < currentFilteredJobs.length - 1) {
        currentJobIndex++;
    } else {
        return;
    }
...
function updateJobNavButtons() {
    setTimeout(() => {
        ...
        if (prevBtn && nextBtn) {
            ...
            nextBtn.disabled = currentJobIndex >= currentFilteredJobs.length - 1;
        }
    }, 100);
}
...
function getCurrentFilteredJobs() {
    ...
    return visibleJobs.length > 0 ? visibleJobs : jobListings;
}
    ...
    if (job) {
        ...
        if (confirm(confirmMessage)) {
            ...
            if (window.showSuccessMessage) {
                showSuccessMessage('지원이 완료되었습니다! 이력서가 담당자에게 전송되었습니다.');
            } else {
                alert(
        ...
        if (confirm(confirmMessage)) {
            ...
            if (navigator.clipboard) {
                navigator.clipboard.writeText(phoneNumber[0]).then(() => {
                    if (window.showSuccessMessage) {
                        showSuccessMessage(`전화번호가 클립보드에 복사되었습니다! ${phoneNumber[0]}`);
                    } else {
                        alert(
                            `전화번호가 클립보드에 복사되었습니다!\n\n📞 ${phoneNumber[0]}\n\n지금 바로 전화를 걸어보세요.`
                        );
                ...
                if (window.showInfoMessage) {
                    showInfoMessage(`전화번호: ${phoneNumber[0]} - 번호를 복사하여 전화를 걸어보세요.`);
                } else {
...
function filterJobs(jobs) {
    return jobs.filter((job) => {
        ...
        if (currentFilters.experience !== 'all') {
            let expMatch = false;
            if (currentFilters.experience === 'new' && job.experience.includes('신입'))
                expMatch = true;
            if (currentFilters.experience === '1-3' && job.experience.includes('1~3년'))
                expMatch = true;
            if (
                currentFilters.experience === '3-5' &&
                (job.experience.includes('3~5년') || job.experience.includes('3~7년'))
            )
                expMatch = true;
            if (
                currentFilters.experience === '5-10' &&
                (job.experience.includes('5~10년') || job.experience.includes('5년 이상'))
            )
                expMatch = true;
            if (currentFilters.experience === '10+' && job.experience.includes('10년'))
                expMatch = true;
        ...
        if (currentFilters.employment !== 'all') {
            ...
            // title에서 모집유형 키워드 검색 (직급별 매칭)
            if (currentFilters.employment === '본부장' && job.title.includes('본부장'))
                empMatch = true;
            if (currentFilters.employment === '팀장' && job.title.includes('팀장')) empMatch = true;
            if (currentFilters.employment === '차장' && job.title.includes('차장')) empMatch = true;
            if (currentFilters.employment === '과장' && job.title.includes('과장')) empMatch = true;
            if (currentFilters.employment === '대리' && job.title.includes('대리')) empMatch = true;
        ...
        if (currentFilters.keyword !== 'all') {
            ...
            if (currentFilters.keyword === 'private-sale') {
                // 민간분양: 민간분양, 민간, 일반분양 관련 키워드
                keywordMatch =
                    job.title.includes('민간분양') ||
                    job.description.includes('민간분양') ||
                    job.title.includes('일반분양') ||
                    job.description.includes('일반분양') ||
                    job.title.includes('민간') ||
                    job.description.includes('민간') ||
                    job.tags.some(
                        (tag) =>
                            tag.includes('민간분양') ||
                            tag.includes('일반분양') ||
            ...
            } else if (currentFilters.keyword === 'public-sale') {
                // 공공분양: 공공분양, 공공주택, LH분양 관련 키워드
                keywordMatch =
                    job.title.includes('공공분양') ||
                    job.description.includes('공공분양') ||
                    job.title.includes('공공주택') ||
                    job.description.includes('공공주택') ||
                    job.title.includes('LH') ||
                    job.description.includes('LH') ||
                    job.title.includes('한국토지주택공사') ||
                    job.description.includes('한국토지주택공사') ||
                    job.tags.some(
                        (tag) =>
                            tag.includes('공공분양') ||
                            tag.includes('공공주택') ||
            ...
            } else if (currentFilters.keyword === 'rental') {
                // 임대: 임대, 렌탈, 리스 관련 키워드
                keywordMatch =
                    job.title.includes('임대') ||
                    job.description.includes('임대') ||
                    job.title.includes('렌탈') ||
                    job.description.includes('렌탈') ||
                    job.title.includes('리스') ||
                    job.description.includes('리스') ||
                    job.title.includes('공공지원민간임대') ||
                    job.description.includes('공공지원민간임대') ||
                    job.title.includes('장기임대') ||
                    job.description.includes('장기임대') ||
                    job.tags.some(
                        (tag) =>
                            tag.includes('임대') || tag.includes('렌탈') || tag.includes('리스')
                    );
            } else if (currentFilters.keyword === 'move-in') {
                // 입주: 입주, 이사, 입주관리, 사후관리 관련 키워드
                keywordMatch =
                    job.title.includes('입주') ||
                    job.description.includes('입주') ||
                    job.title.includes('이사') ||
                    job.description.includes('이사') ||
                    job.title.includes('입주관리') ||
                    job.description.includes('입주관리') ||
                    job.title.includes('사후관리') ||
                    job.description.includes('사후관리') ||
                    job.title.includes('입주자관리') ||
                    job.description.includes('입주자관리') ||
                    job.tags.some(
                        (tag) =>
                            tag.includes('입주') ||
                            tag.includes('이사') ||
            ...
            } else if (currentFilters.keyword === 'others') {
                // 기타: 컨설팅, 기획, 마케팅, 개발사업, 기타 업무
                keywordMatch =
                    job.title.includes('컨설팅') ||
                    job.description.includes('컨설팅') ||
                    job.title.includes('마케팅') ||
                    job.description.includes('마케팅') ||
                    job.title.includes('개발사업') ||
                    job.description.includes('개발사업') ||
                    job.title.includes('사업기획') ||
                    job.description.includes('사업기획') ||
                    job.title.includes('투자') ||
                    job.description.includes('투자') ||
                    job.tags.some(
                        (tag) =>
                            tag.includes('컨설팅') ||
                            tag.includes('마케팅') ||
        ...
        if (currentFilters.status.length > 0) {
            ...
            for (const status of currentFilters.status) {
                if (status === 'recruiting' && job.deadline !== 0 && !job.isUrgent)
                    statusMatch = true;
...
function createJobCardHTML(job) {
    ...
    for (let i = 0; i < titleParts.length; i++) {
        if (
            titleParts[i].includes('분양') ||
            titleParts[i].includes('모집') ||
            titleParts[i].includes('채용')
        ) {
            break;
        }
    ...
    // 직급을 더 명확하게 표시
    let position = '대리';
    if (job.title.includes('본부장')) position = '본부장';
    else if (job.title.includes('팀장')) position = '팀장';
    else if (job.title.includes('차장')) position = '차장';
    else if (job.title.includes('과장')) position = '과장';
    else if (job.title.includes('대리')) position = '대리';
...
function renderPinnedBanner() {
    ...
    const bannerHTML = duplicatedJobs
        .map((job) => {
            // 모집유형 결정
            let employmentType = 'general';
            let employmentText = '대리';

            if (job.title.includes('본부장')) {
                employmentType = 'director';
                employmentText = '본부장';
            } else if (job.title.includes('팀장')) {
                employmentType = 'team-manager';
                employmentText = '팀장';
            } else if (job.title.includes('차장')) {
                employmentType = 'deputy-manager';
                employmentText = '차장';
            } else if (job.title.includes('과장')) {
                employmentType = 'section-chief';
                employmentText = '과장';
            } else if (job.title.includes('대리')) {
                employmentType = 'assistant-manager';
                employmentText = '대리';
            }
            ...
            // 등록일 포맷팅
            const registerDate = job.postedDate
            ...
            <div class="pinned-banner-item" data-job-id="${job.id}">
                <div class="pinned-content">
                    ...
                    <div class="pinned-title">${job.title.replace(' 분양기획', '').replace(' 모집', '').replace(' 채용', '')}</div>
...
function handleFormSubmit(e) {
    ...
    if (window.showSuccessMessage) {
        showSuccessMessage('구인공고가 성공적으로 등록되었습니다. 관리자 검토 후 게시됩니다.');
    } else {
...
function renderPagination() {
    const paginationContainer = document.getElementById('planningPagination');
...
function goToPage(page) {
    ...
    currentPage = page;
    renderJobs(currentFilteredJobs); // 기존 렌더링 함수 호출
    renderPagination();
