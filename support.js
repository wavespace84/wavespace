...
function renderFAQ(category = 'all', searchTerm = '', scrollToTop = false) {
    ...
    if (searchTerm && searchTerm.trim()) {
        ...
        // 각 FAQ 항목에 대해 점수 계산
        filteredFAQ = filteredFAQ.map(item => {
    ...
    roomJoinBtns.forEach(btn => {
        ...
        if (!userInfo.isManager) {
            ...
            if (roomType === 'planning') {
                ...
                // 청약상담사방은 청약상담 파트만 가능
                if (userInfo.memberCategory !== 'consultant' && userInfo.memberCategory !== 'business') {
                    btn.disabled = true;
                }
            }
            // 비즈니스방은 모든 회원 가능
        ...
        btn.addEventListener('click', function() {
            ...
            if (roomType === 'planning' && !userInfo.isManager) {
                ...
                setTimeout(() => {
                    ...
                    // 오픈채팅방 참여 문의 자동 선택
                    setTimeout(() => {
                        const openchatInquiryBtn = document.querySelector('.inquiry-type-btn[data-type="오픈채팅방 참여 문의"]');
...
function renderInquiries(status = 'all') {
    ...
    const filtered = status === 'all' 
        ? myInquiriesData 
        : myInquiriesData.filter(i => i.status === status);
    ...
    // 문의 목록 렌더링
    inquiryList.innerHTML = filtered.map(inquiry => {
